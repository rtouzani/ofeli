...
//   or adaptation
   ...
   string fgeom=zz, fmeshback=zz, fmeshout=zz, fmeshr=zz, fmetrix=zz, fmsh=zz;
   string rbb=zz, rBB=zz, wbb=zz, wBB=zz;
   string fMbb=zz, foM=zz, fMBB=zz;
   verbosity = 0;
   char *datargv[128];
   int datargc=1;
   ...
   for (i=1; i<argc; i++)
      if (argv[i]=="-v" && ++i<argc)
         verbosity = atoi(argv[i].c_str());
      else if (argv[i]=="-NoRescaling") 
         Rescaling = 0;
      else if (argv[i]=="-Rescaling") 
         Rescaling = 1;
      else if (argv[i]=="-H" && ++i<argc) 
        ChoiseHessien = atoi(argv[i].c_str());
      else if (argv[i]=="-power" && ++i<argc) 
         power = atof(argv[i].c_str());
      else if (argv[i]=="-nbv" && ++i<argc)
         nbvx = atoi(argv[i].c_str());
      else if (argv[i]=="-nbs" && ++i<argc)
         nbvx = atoi(argv[i].c_str());
      else if (argv[i]=="-g" && ++i<argc)
         fgeom = argv[i];
      else if ((argv[i]=="-thetaquad" || argv[i]=="-ThetaQuad") && ++i<argc)
         costheta = 1 - Abs(cos(Pi*atof(argv[i].c_str())/180.0));
      else if (argv[i]=="-b" && ++i<argc)
         fmeshback = argv[i];
      else if (argv[i]=="-r" && ++i<argc)
         fmeshr = argv[i];
      else if (argv[i]=="-M" && ++i<argc) 
         fmetrix = argv[i];
      else if (argv[i]=="-rbb" && ++i<argc) 
         rbb = argv[i];
      else if (argv[i]=="-rBB" && ++i<argc) 
         rBB = argv[i];
      else if (argv[i]=="-Mbb" && ++i<argc) 
         fMbb = argv[i];
      else if (argv[i]=="-MBB" && ++i<argc) 
         fMBB = argv[i];
      else if (argv[i]=="-wBB" && ++i<argc) 
         wBB = argv[i];
      else if (argv[i]=="-wbb" && ++i<argc) 
         wbb = argv[i];
      else if (argv[i]=="-o" && ++i<argc) 
         fmeshout = argv[i];
      else if (argv[i]=="-intm" && i<argc)
         hinterpole = 0; 
      else if (argv[i]=="-omsh" && ++i<argc)
         fmsh = argv[i];
      else if (argv[i]=="-oM" && ++i<argc)
         foM = argv[i];
      else if (argv[i]=="-coef" && ++i<argc) 
         coef = atof(argv[i].c_str());
      else if (argv[i]=="-err" && ++i<argc) 
         err = atof(argv[i].c_str());
      else if (argv[i]=="-errg" && ++i<argc) 
         errg = atof(argv[i].c_str());
      else if (argv[i]=="-ratio" && ++i<argc) 
         ratio = atof(argv[i].c_str());
      else if (argv[i]=="-hmin" && ++i<argc) 
         hmin = atof(argv[i].c_str());
      else if (argv[i]=="-hminaniso" && ++i<argc) 
         hminaniso = atof(argv[i].c_str());
      else if (argv[i]=="-hmax" && ++i<argc) 
         hmax = atof(argv[i].c_str());
      else if (argv[i]=="-thetamax"  && ++i<argc) 
         cutoffradian = atof(argv[i].c_str())*Pi/180.0;
      else if (argv[i]=="-anisomax" && ++i<argc) 
         anisomax = atof(argv[i].c_str());
      else if (argv[i]=="-maxsubdiv" && ++i<argc) 
         maxsubdiv = atof(argv[i].c_str());
      else if (argv[i]=="-iso")
         iso = 1;
      else if (argv[i]=="-KeepBackVertices")
         KeepBackVertices = 1;
      else if (argv[i]=="-noKeepBackVertices")
         KeepBackVertices = 0;
      else if (argv[i]=="-KeepBackVertices")
         KeepBackVertices = 1;
      else if (argv[i]=="-noKeepBackVertices")
         KeepBackVertices = 0;
      else if (argv[i]=="-2q" || argv[i]=="-2Q")
         allquad = 1;
      else if (argv[i]=="-2")
         allquad = 2;
      else if (argv[i]=="-aniso")
         iso = 0;
      else if (argv[i]=="-RelError")
         AbsError = 0;
      else if (argv[i]=="-AbsError")
         AbsError = 1;
      else if (argv[i]=="-splitpbedge") 
         SplitEdgeWith2Boundary=1;
      else if (argv[i]=="-nosplitpbedge") 
         SplitEdgeWith2Boundary=0;
      else if (argv[i]=="-NbJacobi" && ++i<argc) 
         nbjacoby = atoi(argv[i].c_str());
      else if (argv[i]=="-CutOff" && ++i<argc) 
         CutOff = atof(argv[i].c_str()), AbsError = 0;
      else if (argv[i]=="-NbSmooth" && ++i<argc) 
         NbSmooth = atoi(argv[i].c_str());
      else if (argv[i]=="-omega" && ++i<argc) 
         omega =  atof(argv[i].c_str());
      else {
         cout << " Usage:" << endl;
