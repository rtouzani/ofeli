...
void ComputePartitionBalance(GraphType *graph, int nparts, idxtype *where, float *ubvec)
{
  ...
  if (vwgt == NULL) {
    for (i=0; i<nvtxs; i++)
      kpwgts[where[i]]++;
    ubvec[0] = (float)(nparts*kpwgts[idxamax(nparts, kpwgts)]/(1.0*nvtxs));
  }
  else {
    for (j=0; j<ncon; j++) {
      idxset(nparts, 0, kpwgts);
      ...
      ubvec[j] = (float)(nparts*kpwgts[idxamax(nparts, kpwgts)]/(1.0*idxsum(nparts, kpwgts)));
    }
  }
...
float ComputeElementBalance(int ne, int nparts, idxtype *where)
{
  ...
  balance = (float)(nparts*kpwgts[idxamax(nparts, kpwgts)]/(1.0*idxsum(nparts, kpwgts)));
