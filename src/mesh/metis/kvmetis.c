                         ...
                         int *options, int *volume, idxtype *part)
{
  ...
  for (i=0; i<*nparts; i++) 
    tpwgts[i] = (float)1.0/(float)(*nparts);
                          ...
                          float *tpwgts, int *options, int *volume, idxtype *part)
{
  ...
  ctrl.CoarsenTo = (int)amax((*nvtxs)/(40*log(*nparts)/log(2.)), 20*(*nparts));
  ...
  *volume = MlevelVolKWayPartitioning(&ctrl, &graph, *nparts, part, tpwgts, (float)1.03);
