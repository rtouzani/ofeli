                          ...
                          float ubfactor, int npasses, int ffactor)
{
  ...
  idxtype *where, *pwgts, *perm, *bndind, *minwgt, *maxwgt, *itpwgts, *updind, *marker, *phtable;
    ...
    for (nmoves=iii=0; iii<graph->nbnd; iii++) {
      ...
      if (myrinfo->gv >= 0) { /* Total volume gain is too high */
        ...
        *======================================================================*/
        ...
        /* CheckVolKWayPartitionParams(graph, nparts); */
      }
    }
                           ...
                           float ubfactor, int npasses)
{
  ...
  int from, to, vwgt;
  idxtype *where, *pwgts, *perm, *moved, *bndind, *minwgt, *maxwgt, *itpwgts, *updind, *marker, *phtable;
  VEDegreeType *myedegrees;
  VRInfoType *myrinfo;
  PQueueType queue;
...
{
  int ii, iii, i, j, k, kk, me, other;
  idxtype *xadj, *vsize, *adjncy, *where;
...
* This function computes the initial id/ed
**************************************************************************/
void CheckVolKWayPartitionParams(GraphType *graph, int nparts)
{
...
{
  int i, ii, j, jj, k, me=0, nvtxs, tvwgt, first, last, nleft, ncmps, cwgt, ncand, other, target, deltawgt;
