...
void MocRefine2Way(CtrlType *ctrl, GraphType *orggraph, GraphType *graph, float *tpwgts, float ubfactor)
{
  ...
  for (;;) {
    ...
    switch (ctrl->RType) {
      case RTYPE_FM:
        MocBalance2Way(ctrl, graph, tpwgts, (float)1.03);
  ...
  MocBalance2Way(ctrl, graph, tpwgts, (float)1.01);
  MocFM_2WayEdgeRefine(ctrl, graph, tpwgts, 8); 
...
**************************************************************************/
void MocAllocate2WayPartitionMemory(GraphType *graph)
{
  int nvtxs, ncon;
...
void MocProject2WayPartition(CtrlType *ctrl, GraphType *graph)
{
  ...
  MocAllocate2WayPartitionMemory(graph);
