       ...
       float *ubvec)
{
  ...
  for (;;) {
    ...
    if (!MocIsHBalanced(graph->ncon, nparts, graph->npwgts, ubvec)) {
      MocComputeKWayBalanceBoundary(graph);
...
**************************************************************************/
void MocAllocateKWayPartitionMemory(GraphType *graph, int nparts)
{
...
void MocProjectKWayPartition(CtrlType *ctrl, GraphType *graph, int nparts)
{
  ...
  MocAllocateKWayPartitionMemory(graph, nparts);
  where = graph->where;
  rinfo = graph->rinfo;
  bndind = graph->bndind;
  bndptr = idxset(nvtxs, -1, graph->bndptr);
...
**************************************************************************/
void MocComputeKWayBalanceBoundary(GraphType *graph)
{
  int i, nvtxs, nbnd;
