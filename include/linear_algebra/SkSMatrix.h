 ...
 */
    SkSMatrix(const Vect<size_t>& ColHt) : _dof(0)
    {
       _fact = false;
       _is_diagonal = false;
       _dof_type = NODE_DOF;
...
/// \brief Copy Constructor
    SkSMatrix(const SkSMatrix<T_>& m) : _dof(m._dof)
    {
       _length = m._length;
       _size = m._size;
       _ch.resize(_size);
 ...
 */
    ...
    void SSet(size_t    i,
              size_t    j,
              const T_& val) { _a[_ch[i-1]+j-i] = val; }
#endif /* DOXYGEN_SHOULD_SKIP_THIS */
 ...
 */
    void Axpy(T_                   a,
              const SkSMatrix<T_>& m) { _a += a * m._a; }
...
/// \brief Get <tt>i</tt>-th row vector.
    Vect<T_> getRow(size_t i) const { return getColumn(i); }
