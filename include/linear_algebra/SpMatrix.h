 ...
 *  @param [in] v Vector to multiply by matrix
 ...
 *     <li>Max. Number of iterations is 1000
 *     <li>Tolerance is 1.e-8
 *  </ul>
 ...
 */
 ...
 private:
   ...
   size_t _dof;
   int _is_dense, _type, _solver, _prec, _max_it;
...
int SpMatrix<T_>::ILUFactorize(const SpMatrix<T_>& A)
{
   ...
   _l_col_ind.resize(_lnnz); _u_col_ind.resize(_unnz);
   _l_row_ptr.resize(_nb_rows+1); _u_row_ptr.resize(_nb_rows+1);
                              ...
                              Vect<T_>& x)
{
   ...
   if (_is_diagonal) {
      for (size_t i=0; i<_length; i++)
         if (_IJ[i].first==_IJ[i].second)
            x[i] = b[i]/_A.coeff(_IJ[i].first,_IJ[i].first);
      return 0;
   }
   ...
   switch (_solver) {
      ...
      case CG_SOLVER:
         if (_prec==IDENT_PREC) {
            ConjugateGradient<SpMat,Lower,IdentityPreconditioner> im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
         }
         else if (_prec==DIAG_PREC) {
            ConjugateGradient<SpMat,Lower,DiagonalPreconditioner<T_> > im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
            ...
            ConjugateGradient<SpMat,Lower,IncompleteLUT<T_> > im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
      ...
      case BICG_STAB_SOLVER:
         if (_prec==IDENT_PREC) {
            BiCGSTAB<SpMat,IdentityPreconditioner> im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
            ...
            BiCGSTAB<SpMat,DiagonalPreconditioner<T_> > im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
            ...
            BiCGSTAB<SpMat,IncompleteLUT<T_> > im;
            im.setTolerance(_toler);
            im.compute(_A);
            y = im.solve(VectorX(b));
            nb_it = im.iterations();
   ...
   switch (_solver) {
      ...
      case CG_SOLVER:
         nb_it = CG(*this,_prec,b,x,_max_it,_toler,1);
         break;

      case BICG_SOLVER:
         nb_it = BiCG(*this,_prec,b,x,_max_it,_toler,1);
         break;

      case CGS_SOLVER:
         nb_it = CGS(*this,_prec,b,x,_max_it,_toler,1);
         break;

      case BICG_STAB_SOLVER:
         nb_it = BiCGStab(*this,_prec,b,x,_max_it,_toler,1);
         break;

      case GMRES_SOLVER:
         nb_it = GMRes(*this,_prec,b,x,5,_max_it,_toler,1);
         break;
   }
#endif
   return nb_it;
